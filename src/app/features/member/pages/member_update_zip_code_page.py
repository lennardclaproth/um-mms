...
class MemberUpdateZipCodePage(Page, metaclass=AutoWire):
    ...
    def render_body(self, size):
        self._app_context.selected_member.zip_code = Input.get_component("Please enter the zip code following the format DDDDXX", {
            'height': 15, 'width': 100, 'y': 3, 'x': 1, 'input_y': 1})

    def perform_action(self):
        return self._action.act(input=self._app_context.selected_member, page=self, attribute='zip_code')
