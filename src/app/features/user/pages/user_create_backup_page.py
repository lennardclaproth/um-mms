...
class UserCreateBackupPage(Page, metaclass=AutoWire):
    ...
    def render_body(self, size):
        self.input_str = Input.get_component("Please enter a name for the backup.", {
            'height': 15, 'width': 100, 'y': 3, 'x': 1, 'input_y': 1})

    def perform_action(self):
        ExportService.backup(f"backup/{self.input_str.decode()}.zip")
