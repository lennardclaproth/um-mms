...
class UserUpdateFirstnamePage(Page, metaclass=AutoWire):

    def __init__(self, engine: Engine, app_context: AppContext):
        self._action = engine.registry.get_action(UpdateUserAction)
    ...
    def render_body(self, size):
        self._app_context.selected_user.first_name = Input.get_component("Please enter a first name", {
            'height': 15, 'width': 100, 'y': 3, 'x': 1, 'input_y': 1})

    def perform_action(self):
        return self._action.act(input=self._app_context.selected_user, page=self, attribute='first_name')
